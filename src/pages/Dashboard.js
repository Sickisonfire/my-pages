...
export const Dashboard = () => {
  ...
  return (
    <div tw='right-0 left-0 bottom-0 top-16 p-4 absolute flex items-start'>
      ...
      <button tw='w-72 bg-gray-400 bg-opacity-20 hover:bg-opacity-40 text-sm text-gray-700 p-2 pl-4 rounded-lg text-left'>
...
const List = ({ title }) => {
  ...
  return (
    <div tw='p-2 bg-white  w-72 rounded-lg mr-2 max-h-full flex flex-col'>
      <h2 tw='text-sm font-bold mb-2 mx-2 text-gray-700'>{title}</h2>
      <div tw='overflow-y-auto overflow-x-hidden'>
        {items && items.map((item) => <ListItem key={item.id} title={item.title} />)}
      </div>
      {createItemState ? (
        <>
          <input
...
const ListItem = ({ title }) => {
  return (
    <button tw='w-full text-left mb-2'>
